...
//             Envio             //
///////////////////////////////////
void main_crearTEnvio(TEnvio &envio, TCarritoProductos carrito);
void main_liberarTEnvio(TEnvio &envio);
void main_obtenerCarritoProductosTEnvio(TEnvio &envio);
void main_obtenerFechaTEnvio(TEnvio &envio);
void main_imprimirTEnvio(TEnvio &envio);

///////////////////////////////////
//           ColaEnvios          //
///////////////////////////////////
void main_crearTColaEnvios(TColaEnvios &colaEnvios);
void main_encolarEnvioTColaEnvios(TColaEnvios &colaEnvios, TEnvio &envio);
void main_cantidadTColaEnvios(TColaEnvios &colaEnvios);
void main_imprimirTColaEnvios(TColaEnvios &colaEnvios);
void main_desencolarTColaEnvios(TColaEnvios &colaEnvios);
void main_liberarTColaEnvios(TColaEnvios &colaEnvios);
void main_invertirPrioridadTColaEnvios(TColaEnvios &colaEnvios);
void main_masPrioritarioTColaEnvios(TColaEnvios &colaEnvios);
void main_maxTColaEnvios(TColaEnvios &colaEnvios);

///////////////////////////////////
//           Queja               //
///////////////////////////////////
void main_crearTQueja(TQueja &queja, TProducto &producto, TCliente &cliente);
void main_imprimirTQueja(TQueja &queja);
void main_liberarTQueja(TQueja &queja);
void main_fechaTQueja(TQueja &queja);

///////////////////////////////////
//      tablaQuejas        //
///////////////////////////////////
void main_crearTablaQuejas(TablaQuejas &tablaQuejas);
void main_agregarQuejaTablaQuejas(TablaQuejas &tablaQuejas,
                                         TQueja &queja);
void main_imprimirTablaQuejas(TablaQuejas &tablaQuejas);
void main_perteneceQuejaTablaQuejas(
    TablaQuejas &tablaQuejas);
void main_obtenerQuejaTablaQuejas(TablaQuejas &tablaQuejas);
void main_liberarTablaQuejas(TablaQuejas &tablaQuejas);
...
int main() {
  ...
  // tarea4
  TEnvio envio = NULL;
  TColaEnvios colaEnvios = NULL;
  TQueja queja = NULL;
  TablaQuejas tablaQuejas = NULL;
  ...
  while (seguir) {
    ...
    if (cmd_es("Fin", cmd)) { // Termina el programa
      ...
      ///////////////////////////////////
                   ...
                   "crearClientesABB",
                   cmd)) { // Crea un grupo y la almacena en la variable grupo
      ...
      //            Envio              //
      ///////////////////////////////////
    } else if (cmd_es("crearEnvio", cmd)) {
      main_crearTEnvio(envio, coleccionProductos);
      coleccionProductos = NULL;
    } else if (cmd_es("liberarTEnvio", cmd)) {
      main_liberarTEnvio(envio);
    } else if (cmd_es("obtenerCarritoProductosTEnvio", cmd)) {
      main_obtenerCarritoProductosTEnvio(envio);
    } else if (cmd_es("obtenerFechaTEnvio", cmd)) {
      main_obtenerFechaTEnvio(envio);
    } else if (cmd_es("imprimirTEnvio", cmd)) {
      main_imprimirTEnvio(envio);

      ///////////////////////////////////
      //           ColaEnvios          //
      ///////////////////////////////////
    } else if (cmd_es("crearColaEnvios", cmd)) {
      main_crearTColaEnvios(colaEnvios);
    } else if (cmd_es("imprimirColaEnvios", cmd)) {
      main_imprimirTColaEnvios(colaEnvios);
    } else if (cmd_es("encolarEnvioColaEnvios", cmd)) {
      main_encolarEnvioTColaEnvios(colaEnvios, envio);
    } else if (cmd_es("cantidadColaEnvios", cmd)) {
      main_cantidadTColaEnvios(colaEnvios);
    } else if (cmd_es("desencolarColaEnvios", cmd)) {
      main_desencolarTColaEnvios(colaEnvios);
    } else if (cmd_es("liberarColaEnvios", cmd)) {
      main_liberarTColaEnvios(colaEnvios);
    } else if (cmd_es("invertirPrioridadColaEnvios", cmd)) {
      main_invertirPrioridadTColaEnvios(colaEnvios);
    } else if (cmd_es("masPrioritarioColaEnvios", cmd)) {
      main_masPrioritarioTColaEnvios(colaEnvios);
    } else if (cmd_es("maxColaEnvios", cmd)) {
      main_maxTColaEnvios(colaEnvios);

      ///////////////////////////////////
      //           Queja               //
      ///////////////////////////////////
    } else if (cmd_es("crearQueja", cmd)) {
      main_crearTQueja(queja, producto, cliente);
    } else if (cmd_es("imprimirQueja", cmd)) {
      main_imprimirTQueja(queja);
    } else if (cmd_es("liberarQueja", cmd)) {
      main_liberarTQueja(queja);
    } else if (cmd_es("fechaQueja", cmd)) {
      main_liberarTQueja(queja);

      ///////////////////////////////////
      //      tablaQuejas        //
      ///////////////////////////////////
    } else if (cmd_es("crearTablaQuejas", cmd)) {
      main_crearTablaQuejas(tablaQuejas);
    } else if (cmd_es("agregarQuejaTablaQuejas", cmd)) {
      main_agregarQuejaTablaQuejas(tablaQuejas, queja);
      queja = NULL;
    } else if (cmd_es("imprimirTablaQuejas", cmd)) {
      main_imprimirTablaQuejas(tablaQuejas);
    } else if (cmd_es("perteneceQuejaTablaQuejas", cmd)) {
      main_perteneceQuejaTablaQuejas(tablaQuejas);
    } else if (cmd_es("obtenerQuejaTablaQuejas", cmd)) {
      main_obtenerQuejaTablaQuejas(tablaQuejas);
    } else if (cmd_es("liberarTablaQuejas", cmd)) {
      main_liberarTablaQuejas(tablaQuejas);
    } else {
...
void main_crearTEnvio(TEnvio &envio, TCarritoProductos carrito) {
  ...
  envio = crearTEnvio(carrito, fecha);
  printf("Envio creado con exito.\n");
}

void main_liberarTEnvio(TEnvio &envio) {
  liberarTEnvio(envio);
  printf("Envio liberado con exito.\n");
}

void main_obtenerCarritoProductosTEnvio(TEnvio &envio) {
  TCarritoProductos productos = obtenerCarritoProductosTEnvio(envio);

  printf("Carrito de productos del envio:\n");
  imprimirCarritoProductos(productos);
  printf("\n");
}

void main_obtenerFechaTEnvio(TEnvio &envio) {
  TFecha fecha = obtenerFechaTEnvio(envio);
...
///////////////////////////////////
void main_crearTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios == NULL);
  int cantidad = leerInt();
  colaEnvios = crearTColaEnvios(cantidad);
  printf("Cola de envios creada con exito.\n");
}

void main_encolarEnvioTColaEnvios(TColaEnvios &colaEnvios, TEnvio &envio) {
  assert(envio != NULL);
  encolarEnvioTColaEnvios(colaEnvios, envio);
  envio = NULL;
  printf("Se ha agregado el envio a la cola de envios de forma exitosa.\n");
}

void main_cantidadTColaEnvios(TColaEnvios &colaEnvios) {
  int cant = cantidadTColaEnvios(colaEnvios);
  printf("La cantidad de envios en la cola es %d\n", cant);
}

void main_imprimirTColaEnvios(TColaEnvios &colaEnvios) {
  imprimirTColaEnvios(colaEnvios);
}

void main_desencolarTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios != NULL);
  TEnvio envioAux = desencolarTColaEnvios(colaEnvios);
  imprimirTEnvio(envioAux);
  liberarTEnvio(envioAux);
}

void main_liberarTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios != NULL);
  liberarTColaEnvios(colaEnvios);
  assert(colaEnvios == NULL);
  printf("La cola de envios se ha liberado exitosamente.\n");
}

void main_invertirPrioridadTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios != NULL);
  invertirPrioridadTColaEnvios(colaEnvios);
  printf("Se ejecuto invertirPrioridadTColaEnvios exitosamente.\n");
}

void main_masPrioritarioTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios != NULL);
  TEnvio envio = masPrioritarioTColaEnvios(colaEnvios);
  printf("El envio con mayor prioridad de la cola de envios es:\n");
  imprimirTEnvio(envio);
}

void main_maxTColaEnvios(TColaEnvios &colaEnvios) {
  assert(colaEnvios != NULL);
  int cantidadMax = maxTColaEnvios(colaEnvios);
  printf("La cantidad maxima de envios en la cola de envios es: %d\n",
         cantidadMax);
}
...
void main_crearTQueja(TQueja &queja, TProducto &producto, TCliente &cliente) {
  ...
  leerChars(comentario);

  queja = crearTQueja(fecha, producto, cliente, comentario);
  printf("Queja creada exitosamente.\n");
...
void main_liberarTQueja(TQueja &queja) {
  ...
  liberarTQueja(queja);
  assert(queja == NULL);
  printf("Queja liberada exitosamente.\n");
}

void main_fechaTQueja(TQueja &queja) {
  assert(queja != NULL);
  TFecha fechaQueja = fechaTQueja(queja);
  printf("La fecha de la queja es: ");
  imprimirTFecha(fechaQueja);
  printf("\n");
}
...
///////////////////////////////////
void main_crearTablaQuejas(TablaQuejas &tablaQuejas) {
  assert(tablaQuejas == NULL);
  int cantidadEstimada = leerInt();
  tablaQuejas = crearTablaQuejas(cantidadEstimada);
  printf("tablaQuejas creado exitosamente.\n");
}
void main_agregarQuejaTablaQuejas(TablaQuejas &tablaQuejas,
                                         TQueja &queja) {
  assert(tablaQuejas != NULL);
  assert(queja != NULL);
  agregarQuejaTablaQuejas(tablaQuejas, queja);
  queja = NULL;
  printf("Queja agregada a tablaQuejas exitosamente.\n");
}

void main_imprimirTablaQuejas(TablaQuejas &tablaQuejas) {
  assert(tablaQuejas != NULL);
  imprimirTablaQuejas(tablaQuejas);
}
...
void main_perteneceQuejaTablaQuejas(
  ...
  bool pertenece = perteneceQuejaTablaQuejas(tablaQuejas, fecha);
  printf("Una queja con fecha ");
  imprimirTFecha(fecha);
  if (pertenece) {
    printf(" pertenece a la tabla");
  } else {
    printf(" NO pertenece a la tabla");
  }
  printf("\n");
  liberarTFecha(fecha);
}

void main_obtenerQuejaTablaQuejas(
    TablaQuejas &tablaQuejas) {
  assert(tablaQuejas != NULL);

  TFecha fecha;
  leerFecha(fecha);
  TQueja queja_aux = obtenerQuejaTablaQuejas(tablaQuejas, fecha);
  if (queja_aux == NULL) {
    printf("No se puede obtener la queja de fecha ");
    imprimirTFecha(fecha);
    printf(" porque NO pertenece a la tabla.\n");
  } else {
    imprimirTQueja(queja_aux);
  }
  liberarTFecha(fecha);
}
...
void main_liberarTablaQuejas(TablaQuejas &tablaQuejas) {
  assert(tablaQuejas != NULL);
  liberarTablaQuejas(tablaQuejas);
